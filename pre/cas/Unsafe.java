...
public final class Unsafe {
     ...
     * <code>false</code> if not
		public final int getAndAddInt(Object o, long offset, int delta) {
			int v;
			do {
				v = getIntVolatile(o, offset);//先获取旧值
			} while (!compareAndSwapInt(o, offset, v, v + delta));
			return v;
			}
	 *	obj 就是这个atomicInteger的地址
	 *  offset 就是atomicInteger里面的value的值的偏移量 即通过
	 * 	obj +offset 就能找到value在内存中的位置
    ...
    public final int getAndAddInt(Object o, long offset, int delta) {
        int v;
        do {
            v = getIntVolatile(o, offset);//先获取期望值
        } while (!compareAndSwapInt(o, offset, v, v + delta));
		//如果比较和交换失败 就会一直重试，直到成功为止
        return v;
    }
